...
static void mp2650_handle_wdt_for_unwakelock_charge(bool is_suspend){

    struct oppo_chg_chip *p_occ = mp2650_chg_ops.occ;
    static bool is_wdt_disable = false;

    if(!p_occ) {
        chg_err("handle_wdt_for_unwakelock_charge: p_occ null.\n");
        return;
    }
    //chg_err("mp2650_handle_wdt_for_unwakelock_charge: is_suspend:%d is_wdt_disable:%d.unwakelock_chg:%d\n", is_suspend, is_wdt_disable, p_occ->unwakelock_chg);

    if(!p_occ->unwakelock_chg)
        return;

    if(!p_occ->charger_exist)
        return;

    if(is_suspend && !is_wdt_disable){
        mp2650_set_wdt_timer(REG09_MP2650_WTD_TIMER_DISABLE);
        is_wdt_disable = true;
    }else if(!is_suspend && is_wdt_disable){
        mp2650_set_wdt_timer(REG09_MP2650_WTD_TIMER_40S);
        is_wdt_disable = false;
    }
}
...
static int mp2650_pm_resume(struct device *dev)
{
    ...
    */
    mp2650_handle_wdt_for_unwakelock_charge(false);
...
static int mp2650_pm_suspend(struct device *dev)
{
    ...
    mp2650_handle_wdt_for_unwakelock_charge(true);
